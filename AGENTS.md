...
- `dev`是开发分支, 新功能/修改 BUG 必须从 `dev` 新建`feat|fix|...`分支.

- `main` 是稳定主分支, 只有人工确认代码版本稳定, 则合并`dev`分支, 作为稳定版本,

- 禁止使用 `codex/xx` | `claude/xxx` 的前缀创建和提交分支名称.

- `feat|fix|...`分支开发测试完成后, 必须及时合并回本地 `dev` 并删除分支, 避免多分支长期存在导致重复开发、重复冲突和分支列表累积过深.

- 远程 `dev` 推送必须设置严格：只有本地功能分支已经完整合并到 `dev`、测试 / 检查无异常、工作区干净并确认本地 `dev` 状态正确时, 才能推送到远程 `dev` 分支.

- 如果本地功能分支尚未完全合并、测试未通过、工作区不干净、存在冲突或状态不确定, 禁止私自推送远程 `dev`；此时确实需要远程同步, 只能推送当前功能分支到远程.
