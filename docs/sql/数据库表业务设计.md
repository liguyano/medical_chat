...
- 医护端可以维护患者身份与联系信息，但接口不得返回身份证密文；身份证号只允许在录入或重新修改时提交。
- 同一个患者可以有多次住院记录。
...
| `admission_source` | 入院来源，例如门诊、急诊、转院 |
| `nursing_level` | 当前护理级别 |
| `insurance_type` | 医保/费用类别 |
| `allergy_summary` | 当前过敏史安全摘要 |

业务说明：

- 科室、病区和诊断需要保存当时快照，不能只关联会变化的主数据。
- 一次住院下可以创建多个护理任务。
- 本表保存当前住院快照，不建设完整的转科、转床、医嘱和费用历史。
...
| `preparation_status` | AI 首问准备总状态：`not_required`、`queued`、`running`、`ready`、`failed` |
| `preparation_stage` | AI 首问准备当前阶段：Schedule 规划、Dialog 预热或首问生成 |
| `preparation_error` | 最近一次 AI 首问准备失败原因 |
| `preparation_attempt` | 医护手动重新准备次数 |
| `preparation_detail` | 三阶段状态及量表问题、首问等输出快照 |
| `patient_visible_at` | AI 首问准备完成并向患者端发布任务的时间；为空表示患者不可见 |
...
- 任务状态决定护士工作台中的待办。
- AI 对话任务创建后先保持患者不可见，按 `Schedule prepare → Dialog preheat → Dialog opening`
  顺序准备；首问消息落库后才写入 `patient_visible_at`，准备失败时由医护端重试原任务。
...
| `validation_rule` | 数值范围、格式等校验规则；`manual_required` 为布尔人工采集标记，默认 false |

人工采集标记约定（2026-09-15）：`validation_rule.manual_required=true` 表示此题由医护人工采集。该标记复用现有 JSONB，不增加表或列，不写入 `assessment_answer`。患者问卷与 AI 问答跳过该题，页面无真实答案时显示“等待人工”，人工数与患者/AI 已答数分开。配置标记不会触发呼叫护士接口、通知或 `handoff_requested`；完成患者/AI 采集不等于完成医护最终复核。未填写的人工计分题不能按 0 分生成总分或风险结论。
| `sort_no` | 排序 |
...
| `handled_by` | 处理人 |
| `handled_at` | 处理时间 |
| `source_invocation_id` | 来源调用编号；同一会话内唯一，用于识别同一次患者操作或同一个 Agent `call_id` 的重复交付 |

业务说明：

- “患者说青霉素过敏，系统追问并提醒以后主动告知医生”应至少产生风险事件、追问事件和宣教事件。
- 该表为患者画像、护理计划和科研分析提供可统计数据。
- `source_invocation_id` 只消除同一次调用在 HTTP、SSE 或 Worker 链路中的重复交付；不同 Agent `call_id` 即使工具名和参数相同，也必须作为两次独立调用保留。
...
### 12.4 `assessment_report` 评估报告

业务目的：保存护士最终复核后生成的量表综合评估报告，并保留可追溯历史版本。
...
| `report_no` | 报告业务编号 |
| `task_id` | 来源护理任务 |
| `version_no` | 任务内递增报告版本 |
| `report_status` | 已生成、已确认 |
| `source_submission_ids` | 报告采用的最终确认、护士或AI提交ID |
| `source_snapshot` | 生成时量表答案、分数、风险和会话摘要事实快照 |
| `report_content` | 通过结构化校验的LLM综合报告内容 |
| `generated_by` | 生成模型 |
| `generated_at` | 生成时间 |
| `confirmed_by` | 确认护士 |
| `confirmed_at` | 确认时间 |
...
- 重新生成报告必须新增版本，不覆盖历史报告。
- 量表答案和分数由业务规则计算并保存为事实快照，LLM不得自行改写。
- 报告只能在护士最终复核后生成，确认操作只作用于当前报告版本。
...
| 结果应用 | `patient_profile_snapshot`、`nursing_plan`、`nursing_plan_item`、`assessment_report` |
