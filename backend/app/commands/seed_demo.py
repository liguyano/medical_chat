...
作用：幂等准备文本对话闭环体验所需的全部种子数据——
      1) 5 位在院患者及住院记录；
      2) 5 张真实量表（导入并发布为“已发布”版本，供加载器读取）；
      3) 关键词交互规则（抽烟/喝酒/过敏等，触发追问约束）。
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="导入第一期演示数据（患者/量表/关键词规则）")
    parser.add_argument(
        "--scales-directory",
        type=Path,
    ...
    # 2) 真实量表（导入并发布，供 AssessmentQuestionLoader 读取“已发布”版本）
