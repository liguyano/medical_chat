...
def dialog_agent_preheat(self, session_id: str, patient_info: dict, task_config: dict):
    ...
    async def _run_preheat():
        ...
        try:
            ...
            dialog_llm_config = config.get_llm_config("dialog_agent")
            if not dialog_llm_config:
                logger.error("[Dialog Agent] 未找到 dialog_agent LLM 配置")
            ...
            if engine_type == "doubao":
                ...
                engine = DoubaoVoiceEngine(
                    api_key=dialog_llm_config["api_key"],
                    model=dialog_llm_config["model"],
                    ws_url=dialog_llm_config["api_base"],
                    timeout=dialog_llm_config.get("timeout", 30.0),
                )
                ...
                engine = TextChatEngine(
                    api_key=dialog_llm_config["api_key"],
                    model=dialog_llm_config["model"],
                    api_base=dialog_llm_config["api_base"],
                    timeout=dialog_llm_config.get("timeout", 30.0),
                )
            ...
            logger.error(f"[Dialog Agent] 预热失败: {e}", exc_info=True)
            raise
