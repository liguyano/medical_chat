...
7. **明确未知也是有效答案**：患者针对当前题明确表示“不知道”“不清楚”“记不清了”“不记得”“忘了”“说不准”“不确定”等时，表示患者已经回答了该题，只是答案未知。此时必须返回当前题候选，不要返回空 answers，也不要继续逼问。value 使用简短的患者未知表述（如“不知道”或“不清楚”），evidence 保留患者原话；confidence 表示你对“患者明确不知道”这一事实的把握，而不是对临床数值的把握。

## 上下文短回答示例
- 护理人员实际问：“通道能顺畅通行吗？”；患者答：“没问题啊。”；对应题目正向选项为“是”时，应返回该题目的“是”选项
- 护理人员实际问：“过去三个月体重下降了多少？”；患者答：“这个我真不清楚。”；应返回该题候选，value 可为“不清楚”，evidence 为“这个我真不清楚”，不能因为题目原本是数值题就返回空 answers
- 示例只说明应结合语境判断，不是关键词规则；如果患者没有表达任何可确定事实、也没有明确表示未知，才返回空 answers
...
- 若患者明确回答“不知道/不清楚/记不清/不确定”等，允许暂时不遵循原题的数字、布尔、日期或选项值格式，直接返回未知文本；后端会以“未知答案”安全保存
- 只有既无法确认实际答案、患者也没有明确表示未知时，才返回 {{"answers": []}}；禁止猜测或返回“待人工确认”
"""
...
def build_user_prompt(
    ...
    for turn_data in new_dialog:
        ...
        if current_question_id is not None:
            ...
            prompt_parts.append(
                ...
                "优先按该题解释，只有回答与问句明显无关时才不要填写。患者若明确说不知道、"
                "不清楚、记不清或不确定，也要把该题作为已回答的未知答案返回。"
            )
    ...
    prompt_parts.append(
        ...
        "患者明确表示不知道/不清楚/记不清/不确定时，这本身就是该题的有效答案，必须返回候选；"
        "只有患者既没有给出实际答案、也没有明确表示未知时才返回空 answers。"
    )
...
def build_focused_user_prompt(new_dialog: list[dict]) -> str:
    ...
    prompt_parts.extend(
        [
            "结合该题的题目定义和选项语义自由判断；不要对普通短回答做固定关键词映射。",
            "如果患者明确说不知道、不清楚、记不清或不确定，这本身就是有效的未知答案，应返回一个候选，不要继续逼问。",
            "能够可靠判断实际答案或明确未知时返回一个候选；其他情况仍无法判断时返回空 answers。",
        ]
    )
    return "\n".join(prompt_parts)
