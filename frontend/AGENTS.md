...
- 任务和对话 Store 的持久化键必须按 `mock` / `api` 运行模式隔离；API 模式初始状态为空，禁止注入或恢复 Mock 患者、任务、会话和答案。API 对话快照加载前必须清除当前任务的旧会话状态，加载失败时不得回退 Mock 数据。
...
- 医护端评估报告页必须以持久化 `sourceSnapshot.assessments` 为事实来源展示全部题目，按量表生成能力概览，并以表格呈现患者结果、临床得分和关注状态；AI 综合摘要只作导读，不得改写原始题目结果。
